...
describe('The ev_common helper module',function()
    ...
    it('calls on_sent callback and small amount of data gets send without buffering',function(done)
        local on_sent = async(function(buf)
            assert.is_equal(buf,chunk1)
            done()
          end)
        ...
        local buffered_bytes = send(chunk1,on_sent,on_err)
        assert.is_equal(buffered_bytes,0)
      end)
    
    it('can be stopped and big amount of data gets buffered',function(done)
        local on_sent = async(function(buf)
            assert.is_nil(err or 'should not happen')
          end)
        ...
        local data = string.rep('foo',3000000)
        local buffered_bytes = send(data,on_sent,on_err)
        assert.is_truthy(buffered_bytes >= 0 and buffered_bytes < #data)
        stop()
